...
if (!isActionAccessible($guid, $connection2, '/modules/Records Of Work/recordsOfWork_manageDepartments.php')) {
    ...
    if (!$subcategoryGateway->unique($data, ['className', 'departmentID'])) {
        $URL .= '&return=error7';
        header("Location: {$URL}");
        exit();
    }
    ...
    $logGateway->addLog($session->get('gibbonSchoolYearID'), 'Records Of Work', $session->get('gibbonPersonID'), 'Subcategory Added', ['subcategoryID' => $subcategoryID]);
