...
if (!isActionAccessible($guid, $connection2, '/modules/Records Of Work/recordsOfWork_manageReplyTemplates.php')) {
    ...
    $table->addHeaderAction('add', __('Add'))
        ->setURL('/modules/' . $session->get('module') . '/recordsOfWork_addReplyTemplate.php')
        ->displayLabel();
    ...
    $table->addActionColumn()
        ->addParam('recordsOfWporkReplyTemplateID')
        ->format(function ($riskTemplate, $actions) use ($moduleName) {
            $actions->addAction('edit', __('Edit'))
                ->setURL('/modules/' . $moduleName . '/recordsOfWork_editReplyTemplate.php');

            $actions->addAction('delete', __('Delete'))
                ->setURL('/modules/' . $moduleName . '/recordsOfWork_deleteReplyTemplate.php');
        });
