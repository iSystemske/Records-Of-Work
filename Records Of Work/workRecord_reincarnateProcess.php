...
$_POST['address'] = '/modules/Records Of Work/workRecord_reincarnateProcess.php';
...
if (!isActionAccessible($guid, $connection2, '/modules/Records Of Work/workRecord_view.php')) {
    ...
    if (($issue['gibbonPersonID'] == $gibbonPersonID) || ($related && $techGroupGateway->getPermissionValue($gibbonPersonID, 'undoRecordsChecked'))) {
        $URL .= "/workRecord_discussView.php&workrecordID=$workrecordID";

        $status = 'InReview';
        if ($issue['qualityassuaranceID'] == null) {
            $status = 'Submitted';
        }

        //Write to database
        if (!$issueGateway->update($workrecordID, ['status' => $status])) {
            $URL .= '&return=error2';
            header("Location: {$URL}");
            exit();
        }
        ...
        $logGateway->addLog($session->get('gibbonSchoolYearID'), 'Records Of Work', $gibbonPersonID, 'Record Of Work Reincarnated', $array);
