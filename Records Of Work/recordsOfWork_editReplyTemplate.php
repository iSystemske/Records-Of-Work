...
if (!isActionAccessible($guid, $connection2, '/modules/Records Of Work/recordsOfWork_manageReplyTemplates.php')) {
    ...
    $replyTemplate = $replyTemplateGateway->getByID($recordsOfWporkReplyTemplateID);

    if (empty($replyTemplate)) {
        $page->addError(__('Invalid Reply Template.'));
    } else {
        $form = Form::create('editRiskTemplate', $session->get('absoluteURL') . '/modules/' . $session->get('module') . '/recordsOfWork_editReplyTemplateProcess.php');
        ...
        $row = $form->addRow();
            ...
            $row->addTextfield('name')
                ->setRequired(true)
                ->maxLength(30)
                ->uniqueField('./modules/' . $session->get('module') . '/recordsOfWork_addReplyTemplateAjax.php', ['currentTemplateName' => $replyTemplate['name']])
