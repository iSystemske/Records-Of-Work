...
use Gibbon\Module\RecordsOfWork\Data\Setting;
use Gibbon\Module\RecordsOfWork\Data\SettingManager;
use Psr\Container\ContainerInterface;

function getSettings(ContainerInterface $container) {
    $settingManager = new SettingManager($container->get(SettingGateway::class), 'Records Of Work');
    ...
    $settingManager->addSetting('records0fWorkCategory')
        ->setRenderer(function($data, $row) {
    ...
    $settingManager->addSetting('recordsPriority')
        ->setRenderer(function($data, $row) {
            $row->addTextArea($data['name'])
                ->setValue($data['value']);
        })
    ...
    $settingManager->addSetting('recordsPriorityName')
        ->setRenderer(function($data, $row) {
    ...
    $settingManager->addSetting('qaNotes')
        ->setRenderer(function($data, $row) {
            $row->addCheckbox($data['name'])
                ->checked(intval($data['value']));
        })
