...
$page->breadcrumbs->add(__('Manage QA Groups'));
...
if (!isActionAccessible($guid, $connection2, '/modules/Records Of Work/recordsOfWork_manageTechnicianGroup.php')) {
    ...
    $manageTechnicians = isActionAccessible($guid, $connection2, '/modules/Records Of Work/recordsOfWork_manageTechnicians.php');
    ...
    $canEditDepartment = isActionAccessible($guid, $connection2, '/modules/Records Of Work/recordsOfWork_manageDepartments.php');
    $techGroupData = $techGroupGateway->selectTechGroups()->toDataSet();

    $table = DataTable::create('techGroups');
    $table->setTitle('QA Groups');

    $table->addHeaderAction('add', __('Add'))
            ->setURL('/modules/' . $session->get('module') . '/recordsOfWork_createTechnicianGroup.php');

    $table->addColumn('groupName', __('Group Name'));

    $settingGateway = $container->get(SettingGateway::class);

    if ($departmentGateway->countAll() > 0 && !$settingGateway->getSettingByScope('Records Of Work', 'simpleCategories')) {
            ...
            ->format(function ($techGroup) use ($session, $groupDepartmentGateway, $canEditDepartment) {
                ...
                return implode(', ', array_map(function ($department) use ($session, $canEditDepartment) {
                    if ($canEditDepartment) {
                        return Format::link('./index.php?q=/modules/' . $session->get('module') . '/recordsOfWork_editDepartment.php&departmentID='. $department['departmentID'], $department['schoolYearGroup']);
                    } else {
                        return $department['schoolYearGroup'];
                    }
                }, $departments));
            });
    }
        ...
        ->format(function($row) use ($technicianGateway, $manageTechnicians, $moduleName) {
            ...
            return implode(', ', array_map(function ($row) use ($manageTechnicians, $moduleName) {
                ...
                if ($manageTechnicians) {
                    return Format::link('./index.php?q=/modules/' . $moduleName . '/recordsOfWork_setTechGroup.php&qualityassuaranceID=' . $row['qualityassuaranceID'], $name);
                } else {
                    return $name;
                }
            }, $technicians));
    ...
    $table->addActionColumn()
            ...
            ->format(function ($techGroup, $actions) use ($session, $techGroupData) {
                $actions->addAction('edit', __('Edit'))
                        ->setURL('/modules/' . $session->get('module') . '/recordsOfWork_editTechnicianGroup.php');

                if (count($techGroupData) > 1) {
                    $actions->addAction('delete', __('Delete'))
                            ->setURL('/modules/' . $session->get('module') . '/recordsOfWork_technicianGroupDelete.php');
                }
            });
