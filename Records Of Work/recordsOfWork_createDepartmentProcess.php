...
$URL = $session->get('absoluteURL') . '/index.php?q=/modules/' . $session->get('module') . '/recordsOfWork_createDepartment.php';
...
if (!isActionAccessible($guid, $connection2, '/modules/Records Of Work/recordsOfWork_manageDepartments.php')) {
    ...
    if (empty($schoolYearGroup) || strlen($schoolYearGroup) > 55 || empty($departmentDesc) || strlen($departmentDesc) > 128 || empty($roles)) {
        ...
        if (!$departmentGateway->unique($data, ['schoolYearGroup'])) {
            $URL .= '&return=error7';
            header("Location: {$URL}");
            exit();
        }
        ...
        $logGateway->addLog($session->get('gibbonSchoolYearID'), 'Records Of Work', $session->get('gibbonPersonID'), 'Department Added', ['departmentID' => $departmentID]);
