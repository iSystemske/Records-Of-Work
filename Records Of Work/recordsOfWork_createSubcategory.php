...
if (!isActionAccessible($guid, $connection2, '/modules/Records Of Work/recordsOfWork_manageDepartments.php')) {
    ...
    if (empty($departmentID) || !$departmentGateway->exists($departmentID)) {
        ...
        $form = Form::create('createSubcategory',  $session->get('absoluteURL') . '/modules/' . $session->get('module') . '/recordsOfWork_createSubcategoryProcess.php', 'post');
        ...
        $row = $form->addRow();
            ...
            $row->addTextField('className')
                ->required()
                ->uniqueField('./modules/' . $session->get('module') . '/recordsOfWork_createSubcategoryAjax.php', ['departmentID' => $departmentID])
                ->maxLength(55);
