...
$page->breadcrumbs
    ->add(__('Manage Departments'), 'recordsOfWork_manageDepartments.php')
...
if (!isActionAccessible($guid, $connection2, '/modules/Records Of Work/recordsOfWork_manageDepartments.php')) {
    ...
    if (empty($department)) {
        ...
        $form = Form::create('createDepartment',  $session->get('absoluteURL') . '/modules/' . $moduleName . '/recordsOfWork_editDepartmentProcess.php', 'post');
        $form->setTitle($department['schoolYearGroup']);
        $form->addHiddenValue('address', $session->get('address'));
        $form->addHiddenValue('departmentID', $departmentID);

        $row = $form->addRow();
            $row->addLabel('schoolYearGroup', __('Department Name'));
            $row->addTextField('schoolYearGroup')
                ->maxLength(55)
                ->uniqueField('./modules/' . $moduleName . '/recordsOfWork_createDepartmentAjax.php', ['currentDepartmentName' => $department['schoolYearGroup']])
                ->required();
        ...
        $row = $form->addRow();  
            $row->addLabel('roles', __('Select Roles'))
                ->description(__('Which roles can create records for this department'));
            $row->addSelect('roles')
        ...
        $table->addHeaderAction('add', __('Create'))
                ...
                ->modalWindow()
                ->displayLabel()
                ->setURL('/modules/' . $moduleName . '/recordsOfWork_createSubcategory.php');

        $table->addColumn('className', __('Name'));
        ...
        $table->addActionColumn()
                ...
                ->format(function ($subcategory, $actions) use ($moduleName) {
                    $actions->addAction('edit', __('Edit'))
                            ->modalWindow()
                            ->setURL('/modules/' . $moduleName . '/recordsOfWork_editSubcategory.php');

                    $actions->addAction('delete', __('Delete'))
                            ->setURL('/modules/' . $moduleName . '/recordsOfWork_deleteSubcategory.php');
                });
