...
if (empty($session->get('gibbonPersonID')) || empty($session->get('gibbonRoleIDPrimary'))
    ...
    if (!empty($currentDepartmentName) && $currentDepartmentName == $schoolYearGroup) {
    	echo 0;
    	die();
    }

    $data = ['schoolYearGroup' => $schoolYearGroup];
    $sql = 'SELECT COUNT(*) FROM qualityAssuaranceDepartments WHERE schoolYearGroup=:schoolYearGroup';
