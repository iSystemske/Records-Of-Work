...
$URL = $session->get('absoluteURL') . '/index.php?q=/modules/' . $session->get('module') . '/recordsOfWork_createTechnician.php';
...
if (!isActionAccessible($guid, $connection2, '/modules/Records Of Work/recordsOfWork_manageTechnicians.php')) {
    ...
    if (empty($person) || !$userGateway->exists($person)
        ...
        $qualityassuaranceID = $technicianGateway->insert($data);
        if ($qualityassuaranceID === false) {
            $URL .= '&return=error2';
            header("Location: {$URL}");
            exit();
        }
        ...
        $logGateway->addLog($session->get('gibbonSchoolYearID'), 'Records Of Work', $session->get('gibbonPersonID'), 'QA Added', ['gibbonPersonID' => $person, 'qualityassuaranceID' => $qualityassuaranceID]);

        //Success 0
        $URL .= "&qualityassuaranceID=$qualityassuaranceID&return=success0";
        header("Location: {$URL}");
        exit();
    }
}
