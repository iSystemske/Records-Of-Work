...
if (!isActionAccessible($guid, $connection2, '/modules/Records Of Work/recordsOfWork_manageTechnicianGroup.php')) {
    ...
    if (empty($groupID) || !$techGroupGateway->exists($groupID)) {
        ...
        $sql = 'SELECT groupID as value, groupName as name FROM qualityAssuaranceGroups WHERE groupID!=:groupID ORDER BY qualityAssuaranceGroups.groupID ASC'; 

        //Make sure that there are other groups aside from the group being deleted
        if ($techGroupGateway->countAll() > 1) {
            $form = DeleteForm::createForm($session->get('absoluteURL') . '/modules/' . $session->get('module') . "/recordsOfWork_technicianGroupDeleteProcess.php?groupID=$groupID", false, false);
            $form->addHiddenValue('address', $session->get('address'));
            $form->setTitle(__($values['groupName']));
            $row = $form->addRow();
                $row->addLabel('group', __('New QA Group'))
